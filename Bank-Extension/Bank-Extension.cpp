...
enum enMainMenuOption{showClientList=1,AddNewClient=2,DeleteClient=3,
	UpdateClientInfo=4,FindClient=5,enTransaction=6,Exit=7};

enum enTransactionMenuOption{enDeposit=1,enWithdraw=2,enTotalBalance=3,enMainMenu=4};
...
void PrintAllClientsData(vector <sClient> vClients)
{
	...
	if (vClients.size() == 0)
	{
		cout << "\t\t\t\t No Clients Available in the systems !\n"; 
	}
	else
	{
...
void PrintAllClientsBalance (vector <sClient> vClients)
{
	sClient Client;
	cout << "\n\t\t\t\t\tClient List (" << vClients.size() << ")Client(s)";
	cout << "\n___________________________________________________";
...
enTransactionMenuOption ChoseTransaction()
{
	short Number = 0;
	cout << "Choose what do you want to do  [1-4] ?" << endl;
	cin >> Number;
	return (enTransactionMenuOption)Number;
}
...
bool WithdrawClientByAccountNumber(string AccountNumber, vector<sClient>& vClients)
{
	...
	if (FindClientByAccountNumber(AccountNumber, vClients, Client))
	{
		...
		if (Answer == 'y' || Answer == 'Y')
		{
			...
			cout << "\n\n Done Successfully your new Balance is ." << Client.AccountBalance << endl;
...
double TotalBanlcne(vector<sClient>& vClients)
{
...
void ShowTotalBanlcneScreen()
{
	vector<sClient> vclient = ReadClientDataFromFile(ClientsFileName);
	PrintAllClientsBalance(vclient);
	cout << "\t\t\t\t Total Banlcne" << TotalBanlcne(vclient) << endl;
...
void PerformTransactionsMenuOption(enTransactionMenuOption TransactionMenuOption)
{
	...
	case enTransactionMenuOption::enTotalBalance:
		system("cls");
		ShowTotalBanlcneScreen();
		GoBackToTransactionsMenu();
		break;
...
void showTransactionsMenu()
{
	...
	cout << "\t[4] Main Menu.\n";
	cout << "=========================================\n";
	PerformTransactionsMenuOption(ChoseTransaction());
}
